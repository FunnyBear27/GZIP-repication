...
// ----------------- private methods ----------------
	...
	void add_one_iterator(TwoBytes& current_code, TwoBytes& value, TwoBytes& max_val) {
		while (!(value == max_val)) {
	...
	void generate_static_huffman() { // RFC 1951 DEFLATE specs - 3.2.6
		...
		current_code[0] = std::byte { 0x01 };
		current_code[1] = std::byte { 0x90 };

		max_val[0] = std::byte { 0x01 };
		max_val[1] = std::byte { 0x00 };

		add_one_iterator(current_code, value , max_val);
		
		// 256 - 279

		current_code[0] = std::byte { 0x00 };
		current_code[1] = std::byte { 0x00 };

		max_val[0] = std::byte { 0x01 };
		max_val[1] = std::byte { 0x18 };
